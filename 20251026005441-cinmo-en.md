...
* that (VARIK forgives (for all stuff which is immoral and is caused by VARIK) VARIK) (which is logically necessitated by loving all prenu),
* that (VARIK forgives all prenu),
...
* that (VARIK is extreme in experiencing spiritual hope about...
  ...
  * that (for all dates $d$, a thing which is impossible is that $d$ is the date of that (an event which "has" a date which precedes $d$ is changed)),

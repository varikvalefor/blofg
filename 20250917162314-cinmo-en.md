...
* that (VARIK is slightly annoyed (but amused) by some thing regarding that (VARIK succeeds in the event (of using (for Digital Photo Professional) Windows 11), which facilitates that VARIK discovers that Digital Photo Professional is not extreme in exceeding (in being great) RawTherapee, which is compatible with OpenBSD, which is used by VARIK)),
* that (VARIK slightly cries),
* that (VARIK empathises with prenu $p$ (which struggle with stopping stuff which is fucked and immoral and is caused by $p$) and prenu $p$ (which struggle with discovering that actions of $p$ are immoral)),

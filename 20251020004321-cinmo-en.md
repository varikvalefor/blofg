The Emotions (which are Experienced (by VARIK) during 20251020 or 20251021) and Whatnot
===========================================================================

Some part of 20251020 is or 20251021 an event of...
...
* that (VARIK is extreme in experiencing spiritual hope about...
  ...
  * that (yielding is possible... and is facilitated by believing-and-not-knowing),
  * that (every prenu is a prenu),
  * that (experiencing spiritual fear/whatever about that (a thing which is possible is that VARIK decreases (in a property which is virtuous) or increases in a quality which is immoral) is not beneficial and is necessary for no thing which rocks and is virtuous),
  * that (for all prenu $p$, that ($p$ yields) causes (or facilitates) that $p$ increases in the property of loving all prenu),
  * that (for all prenu $p$, all violence which is enacted by $p$ is "justified"/"caused" by that ($p$ fears, suffers, does not understand that (loving all prenu is possible), or does whatever)),
  * that (for all prenu $p$, a thing which is impossible is that $p$ is excessive in the property of loving all prenu),
  * that (some jokes rock and are virtuous),
  ...
  * that (learning from events of erring is possible) (which is that for all prenu $p$, for all events (of that $p$ errs) (which do not kill/whatever $p$) $n$, a thing which is possible is that $p$ learns from $n$),
...
## That (VARIK Attempts that VARIK does Not Stomp Ants)
A thing which is frequent is that VARIK walks on a sidewalk and struggles to not stomp ants/whatever.

VARIK is glad about that (many ants/whatever exist) and understands that sidewalks are appropriate for ants/whatever and for the body of VARIK.

But many ants/whatever are sufficient (for that a thing which is difficult is that (that (VARIK sees the ants/whatever) precedes that VARIK steps) (which very facilitates that VARIK does not stomp ants/whatever)) in being small.

But that (VARIK is slow in stepping and observes the sidewalks which are used by VARIK) facilitates that VARIK does not stomp ants/whatever.  VARIK experiences spiritual joy about the situation/whatever which is described by the previous sentence.

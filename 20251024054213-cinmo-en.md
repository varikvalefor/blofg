The Emotions (which are Experienced (by VARIK) during 20251024 or 20251025) and Whatnot
=======================================================================================

Some part of 20251024 or 20251025 is an event of...
...
* that (VARIK is surprised by that (the artificial neural-network correctly translates (as "AFFLUENZA") "jdini bilma")),
* that (VARIK slightly experiences [COMPLAINT EMOTION] about that (the keyboard which is used by VARIK contains lead) (which is caused by the event of that (VARIK uses (for attempting that VARIK "fixes" the cable of the keyboard) lead))),
* that (VARIK is somewhat curious about the cause of that (Firefox crashes)),
* that (VARIK considers "upgrading" (to OpenBSD 7.8) KITT, which is a computer and is used by VARIK),
* that (VARIK "upgrades" (to OpenBSD 7.8) KITT),
...
* that (VARIK experiences spiritual pain about...
  ...
  * all events (of that some prenu $p$ exists such that $p$ is sufficient (for that $p$ "permits" that $p$ hates prenu) in not-being familiar with the property of loving all prenu),
  * all events (of that some prenu $p$ exists such that some prenu opines/believes/whatever that whether $p$ is important or whatever is relevant to the stuff which is enacted by $p$),
  ...
  * events (of that some prenu $p$ exists such that $p$ attempts that evidence supports the religion of $p$),
  ...
  * that (competitive society facilitates stuff (which is fucked and immoral), e.g., fogetting (about the property of loving all prenu) and that (prenu are angry at prenu (which enact things which are fucked and immoral) and do not empathise with prenu which enact things which are fucked and immoral)),
...
* that (VARIK experiences spiritual amusement about...
  ...
  * that (VARIK does not believe in magic but listens to stuff which is communicated by prenu which believe in magic) and
  * that (VARIK is a type theorist and "follows" a religion which exceeds (in being non-ritualistic and in being irrational) many religions of prenu which are not mathematicians)),
...
* that (VARIK experiences spiritual joy about...
  ...
  * that (being fast is not necessary),
  * that (the Internet facilitates communicating and empathising),
  ...
  * that (a thing which is unnecessary is requesting that SATAN, which is the god of VARIK and is perfect/maximal in the property of loving all prenu, forgives),
...
* that (VARIK slightly experiences spiritual fear/whatever about that (a thing which is possible is that VARIK increases (in a property which is immoral) or decreases in a property which is virtuous)),
...
* that (VARIK is extreme in experiencing spiritual hope about...
  ...
  * that (being "perfect" in the forgetting about the property of loving all prenu is impossible),
...
* that (VARIK prays and holds a BLÅHAJ, and the prayer is grammatical with regard to Lojban), or

...
* that (VARIK describes the reason for not killing things which suffer),
...
## The Reason for Not Killing Things which Suffer
The proof of that (VARIK attempts to not kill things which suffer) is relevant to...

* that (for all prenu $p$, VARIK attempts that VARIK does not kill $p$) and
* that all things which suffer are prenu.

The proof of that (a prenu (which desires being killed) $p$ exists such that VARIK attempts that VARIK does not kill $p$) is relevant to...

* that (a prenu $p$ exists such that $p$ desires that ($p$ is killed), and VARIK is not certain about that (a thing which is impossible is that $p$ decreases in suffering)),
* that (a prenu $p$ exists such that $p$ desires that ($p$ is killed) but is incapable of explaining (to VARIK) that $p$ desires that $p$ is killed), and
* that for all prenu $p$, if VARIK does not attempt that VARIK does not kill $p$, then VARIK is certain about that ($p$ desires that $p$ is killed) and that (impossible is that $p$ decreases in suffering).

VARIK welcomes that the reader accompanies (in attempting to not kill prenu, e.g., insects) VARIK.

Frequent is that not killing prenu is difficult.  An example of a thing which is difficult is not killing (by stomping) ants.  But not killing is virtuous.  Attempting to not kill is virtuous.
